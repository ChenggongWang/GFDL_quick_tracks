...
runname=ctl1880s_SST_control_tigercpu_intelmpi_18
echo $runname

output_dir=$workdir

Tracker=/home/cw55/tigress/local/GFDL_QT_tiger/bin/track.exe
Sorter=/home/cw55/tigress/local/GFDL_QT_tiger/python/track_sorter_z1y.py


# get model data files
files=`ls --color=never -1 ${datadir}/POSTP/02*0101.atmos_4xdaily.nc`
nfiles=`echo $files | wc -w`
files=`ls --color=never -1 $files  | sed 's/^\(.*\)$/"\1"/' | tr '\n' ','`
#echo $files

#input namelist for tracker
