...
if startdate is None:
   sd = trackerData[0:1].index[0].to_pydatetime()
else:
   sd = parse(startdate)
   
if enddate is None:
   ed = trackerData[-1:].index[0].to_pydatetime()
else:
   ed = parse(enddate)

sdDispStr = trackerData[0:1].index[0]
edDispStr = trackerData[-1:].index[0]
...
for stormID, thisstorm in stormGroups:
    ...
    dt = (times[1].to_pydatetime() - times[0].to_pydatetime())
